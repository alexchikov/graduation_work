...
with DAG(
    dag_id="moex_process_tqbr_securities_iceberg",
    ...
    wait_raw = S3KeySensor(
        ...
        bucket_key="raw/moex/tqbr_securities/dt={{ ds }}/*",
        wildcard_match=True,
        poke_interval=60,
        timeout=60 * 60 * 12,
    )
    ...
    process = SparkSubmitOperator(
        ...
        application="/home/airflow/dags/apps/tqbr_securities_to_iceberg.py",
        application_args=[
            "--run-date", "{{ ds }}",
    ...
    dq = SparkSubmitOperator(
        ...
        application_args=[
            "--table", "processed.moex_bridge_security_board",
            "--warehouse", "{{ var.value.ICEBERG_WAREHOUSE }}",
        ],
        conf={
            "spark.hadoop.fs.s3a.impl": "org.apache.hadoop.fs.s3a.S3AFileSystem",

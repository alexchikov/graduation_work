...
with DAG(
    dag_id="moex_process_index_iceberg",
    ...
    wait_raw = S3KeySensor(
        ...
        bucket_key="raw/moex/index/dt={{ ds }}/*",
        wildcard_match=True,
        poke_interval=60,
        timeout=60 * 60 * 12,
    )
    ...
    process = SparkSubmitOperator(
        ...
        application="/home/airflow/dags/apps/index_to_iceberg.py",
        application_args=["--run-date", "{{ ds }}", "--bucket", "{{ var.value.AWS_BUCKET }}", "--warehouse", "{{ var.value.ICEBERG_WAREHOUSE }}"],
        conf={
    ...
    dq = SparkSubmitOperator(
        ...
        application_args=["--table", "processed.moex_dim_market", "--warehouse", "{{ var.value.ICEBERG_WAREHOUSE }}"],
        conf={
            "spark.hadoop.fs.s3a.impl": "org.apache.hadoop.fs.s3a.S3AFileSystem",
            "spark.hadoop.fs.s3a.access.key": "{{ var.value.AWS_ACCESS_KEY }}",
            "spark.hadoop.fs.s3a.secret.key": "{{ var.value.AWS_SECRET_KEY }}",

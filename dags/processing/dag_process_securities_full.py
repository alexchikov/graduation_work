...
with DAG(
    dag_id="moex_process_securities_full_iceberg",
    ...
    wait_raw = S3KeySensor(
        ...
        bucket_key="raw/moex/securities_full/dt={{ ds }}/*",
        wildcard_match=True,
        poke_interval=60,
        timeout=60 * 30,
    )
    ...
    process = SparkSubmitOperator(
        ...
        application="/home/airflow/dags/apps/securities_full_to_iceberg.py",
        application_args=[
            "--run-date", "{{ ds }}",
    ...
    dq = SparkSubmitOperator(
        ...
        application_args=[
            "--table", "processed.moex_dim_security",
            "--warehouse", "{{ var.value.ICEBERG_WAREHOUSE }}",
        ],
        conf={
            "spark.hadoop.fs.s3a.impl": "org.apache.hadoop.fs.s3a.S3AFileSystem",
